...
Source (adjusted): https://github.com/MicahChambers/i3-wm-scripts
...
# Define the DMenu arguments as a list.
DMENU_ARGS='-i -fn "Calibri-10" -nb ' + DEFAULT_BACKGROUND + ' -nf ' + DEFAULT_FOREGROUND + ' -sb ' + SELECTED_BACKGROUND + ' -sf ' + SELECTED_FOREGROUND + ' -p Find:'
DMENU_ARGS=DMENU_ARGS.split(" ")
...
# @return
def get_tree():
    p1 = Popen(["i3-msg", "-t", "get_tree"], stdout=PIPE)
...
def focus_window(jumpid):
    if __debug__:
        print(["i3-msg", '[con_id="%i"] focus'%jumpid])

    p1 = Popen(["i3-msg", '[con_id="%i"] focus'%jumpid])
